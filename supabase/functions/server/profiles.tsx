...
const OPTIONAL_COLS = new Set([
  ...
  "is_verified", "verification_status", "contact_public", "email_verified", "phone_verified",
  "primary_role", "secondary_roles", "onboarding_completed", "profile_setup_percentage",
...
function rowToUser(row: any) {
  return {
    ...
    profileSetupCompleted:     row.onboarding_completed      ?? false,
    profileSetupPercentage:    row.profile_setup_percentage  ?? 0,
...
function buildInsertParts(data: any, excluded: Set<string>, isUpdate = false) {
  ...
  const optMap: Record<string, any> = {
    ...
    onboarding_completed:     data.profileSetupCompleted ?? false,
    profile_setup_percentage: data.profileSetupPercentage ?? 0,
