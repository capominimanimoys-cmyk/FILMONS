    ...
    <div className="flex gap-3 justify-center" onPaste={handlePaste}>
      {digits.map((d, i) => (
        <input
          ...
          className={`w-12 h-14 text-center text-2xl font-black rounded-2xl outline-none transition-all border-2 caret-blue-400
            ${d
...
function ResendTimer({ onResend, disabled }: { onResend: () => void; disabled?: boolean }) {
  ...
  return (
    <button onClick={() => { if (!disabled) { onResend(); setSecs(60); } }} disabled={disabled}
    ...
    <div className="fixed inset-0 flex flex-col overflow-hidden">
      ...
      <div className="relative z-10 flex-1 overflow-y-auto px-5 pb-10 space-y-6">
        ...
        <div className="space-y-3">
          <label className="text-[10px] font-bold text-white/40 uppercase tracking-widest block">Phone Number</label>
          ...
          <button
            ...
            {sending
              ...
              : codeSent
                ? <><CheckCircle className="w-4 h-4 text-green-400"/> Code sent — tap to resend</>
        ...
        <div className="flex items-center gap-3">
          ...
          <span className="text-[11px] text-white/30 font-semibold uppercase tracking-wider">Verification Code</span>
          <div className="flex-1 h-px bg-white/10"/>
        </div>
        ...
        <div className="space-y-4">
          <p className="text-xs text-white/40 text-center">
            {codeSent
              ? <>Code sent to <span className="text-white/70 font-semibold">{country.flag} {country.dial} {phone}</span> — enter it below</>
              : 'Enter the 6-digit code after you tap Send above'}
          </p>

          <OtpInput key={codeSent ? 'sent' : 'idle'} value={otp} onChange={setOtp}/>

          {codeSent && <ResendTimer key={phone + codeSent} onResend={handleResend} disabled={sending || verifying}/>}
