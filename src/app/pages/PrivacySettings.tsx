    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="py-4 space-y-5">
        ...
        <Section title={t('notif.marketplace')} icon={<ShoppingBag className="w-4 h-4"/>}>
          <Toggle on={newOrders}   onChange={() => setNewOrders(!newOrders)}     label="New Orders"          sub="Someone booked your listing"/>
          <Toggle on={payments}    onChange={() => setPayments(!payments)}        label="Payments Received"   sub="Transaction confirmations"/>
...
export function PrivacySettings() {
  ...
  const [whoCanSeeFollowers, setWhoCanSeeFollowers] = useState('Everyone');
  const [whoCanSeePortfolio, setWhoCanSeePortfolio] = useState('Everyone');
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="sticky top-14 lg:top-0 z-20 bg-white border-b border-gray-100 px-4 py-3 flex items-center justify-between">
        <div className="flex items-center gap-3">
          {/* Explicit route, not navigate(-1) -- see Settings.tsx's own
              back button comment for why. */}
          <button onClick={() => navigate('/settings')}
            className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-gray-100 transition-colors">
            <ArrowLeft className="w-4 h-4 text-gray-700"/>
          </button>
          <h1 className="text-base font-black text-gray-900">Privacy & Safety</h1>
        </div>
        <button onClick={save} className="text-xs font-bold text-blue-600">Save</button>
      </div>
      ...
      <div className="py-4 space-y-5">
        ...
        <Section title="Profile Visibility" icon={<Eye className="w-4 h-4"/>}>
          <Toggle on={profilePublic}    onChange={() => setProfilePublic(!profilePublic)}       label="Public Profile"         sub="Anyone can view your profile"/>
          <Toggle on={showLocation}     onChange={() => setShowLocation(!showLocation)}         label="Show Location"          sub="Display your city on your profile"/>
          <Toggle on={showOnlineStatus} onChange={() => setShowOnlineStatus(!showOnlineStatus)} label="Show Online Status"     sub="Let others see when you're active"/>
          <Toggle on={showLastSeen}     onChange={() => setShowLastSeen(!showLastSeen)}         label="Show Last Seen"         sub="Display when you were last active"/>
          <SelectRow label="Who can message you" value={whoCanMsg}
            options={['Everyone','Followers Only','No One']} onChange={setWhoCanMsg}/>
          <SelectRow label="Who can see your followers" value={whoCanSeeFollowers}
            options={['Everyone','Followers Only','Only Me']} onChange={setWhoCanSeeFollowers}/>
          <SelectRow label="Who can see your portfolio" value={whoCanSeePortfolio}
            options={['Everyone','Followers Only','Only Me']} onChange={setWhoCanSeePortfolio}/>
        </Section>

        <Section title="Contact Info" icon={<Users className="w-4 h-4"/>}>
          <Toggle on={showEmail} onChange={() => setShowEmail(!showEmail)} label="Show Email Address"    sub="Visible on your public profile"/>
          <Toggle on={showPhone} onChange={() => setShowPhone(!showPhone)} label="Show Phone Number"     sub="Visible on your public profile"/>
        </Section>

        <Section title="Interactions" icon={<UserX className="w-4 h-4"/>}>
          <Toggle on={allowTagging}  onChange={() => setAllowTagging(!allowTagging)}   label="Allow Tagging"    sub="Others can tag you in posts and reels"/>
          <Toggle on={allowMentions} onChange={() => setAllowMentions(!allowMentions)} label="Allow Mentions"   sub="Others can @mention you"/>
          <Toggle on={followersOnly} onChange={() => setFollowersOnly(!followersOnly)} label="Followers-Only Comments" sub="Only followers can comment on your posts"/>
        </Section>

        <Section title="Data & Personalization" icon={<BarChart2 className="w-4 h-4"/>}>
        ...
        <Section title="Account Security" icon={<Shield className="w-4 h-4"/>}>
          <Toggle on={twoFactor}   onChange={() => setTwoFactor(!twoFactor)}     label="Two-Factor Authentication" sub="Extra security on sign-in"/>
          <Toggle on={loginAlerts} onChange={() => setLoginAlerts(!loginAlerts)} label="Login Alerts"              sub="Get notified of new sign-ins"/>
        </Section>
