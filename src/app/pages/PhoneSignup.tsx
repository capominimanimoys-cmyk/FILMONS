/**
 * PhoneSignup — WhatsApp-style sign-up: Phone → OTP → Onboarding.
 * New users land here, verify their number, and continue to the full
 * Onboarding flow. Existing phone users are redirected to PhoneLogin.
 */
...
function OtpInput({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  ...
  useEffect(() => { focus(Math.min(value.length, 5)); }, []); // eslint-disable-line react-hooks/exhaustive-deps
...
export function PhoneSignup() {
  ...
  const handleSendCode = async () => {
    ...
    setIsLoading(true);
    try {
      await authApi.signupWithPhone(fullPhone);
      ...
      if (msg.toLowerCase().includes('already exists')) {
        toast.error('Account already exists', {
          description: 'Sign in with your phone number instead.',
          action: { label: 'Sign in', onClick: () => { captureSnapshot(); navigate('/phone-login'); } },
        });
      } else {
  ...
  return (
    <div className="fixed inset-0 flex flex-col overflow-hidden">
      ...
      <div className="relative z-10 flex items-center gap-3 px-4 pt-14 pb-3">
        <button
          onClick={() => step === 1 ? (captureSnapshot(), navigate('/create-account')) : setStep(1)}
        ...
        {step === 1 && (
          <div className="pt-4 space-y-5">
            ...
            <p className="text-xs text-white/30 text-center -mt-2">
              We'll send a one-time verification code. Standard rates apply.
            </p>
