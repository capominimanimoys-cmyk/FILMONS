...
export function CreateAccount() {
  ...
  const sendVerificationCode = async () => {
    ...
    sessionStorage.setItem('fm_otp_expires', String(Date.now() + 10 * 60 * 1000));
    try {
      const r = await sendEmail(EMAILJS_CONFIG.templates.emailVerification, {
        to_email: email, to_name: name || email.split('@')[0],
  ...
  const verifyAndCreate = async () => {
    ...
    const stored      = sessionStorage.getItem('fm_otp');
    const storedEmail = sessionStorage.getItem('fm_otp_email');
    const expires     = Number(sessionStorage.getItem('fm_otp_expires') || 0);
    if (Date.now() > expires)               { setOtpError('Code expired — request a new one'); setOtpLoading(false); return; }
    if (storedEmail !== email.toLowerCase()){ setOtpError('Email mismatch'); setOtpLoading(false); return; }
    if (otp !== stored)                     { setOtpError('Incorrect code'); setOtpLoading(false); return; }
    sessionStorage.removeItem('fm_otp');
    sessionStorage.removeItem('fm_otp_email');
    sessionStorage.removeItem('fm_otp_expires');
    try {
      ...
      setOtpError(e instanceof Error ? e.message : 'Account creation failed');
    }
    setOtpLoading(false);
  };
              ...
              {accountCreated ? (
                <div className="space-y-5 text-center">
                  ...
                  <div>
                    ...
                    <p className="text-white/50 text-sm mt-1">A confirmation has been sent to</p>
                    <p className="text-blue-400 text-sm font-semibold mt-0.5">{email}</p>
                  </div>
                  <div className="bg-white/5 border border-white/10 rounded-2xl px-4 py-3 text-left space-y-1">
                    <p className="text-xs text-white/40">You can now sign in with:</p>
                    <p className="text-sm text-white font-semibold">{email}</p>
                    <p className="text-xs text-white/30">Your password was set during signup</p>
                  </div>
                  <button
                    onClick={() => navigate('/login')}
                    className="w-full py-4 bg-blue-600 text-white font-black text-sm rounded-2xl hover:bg-blue-700 active:scale-[0.98] transition-all shadow-lg shadow-blue-900/30"
                  >
                    Sign In to Filmons
                  </button>
                </div>
