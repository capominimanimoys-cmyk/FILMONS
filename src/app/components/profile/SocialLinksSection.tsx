...
export function SocialLinksSection({ links, isOwner, onEdit, creatorId, actorId }: {
  links: SocialLinksData; isOwner: boolean; onEdit?: () => void;
  /** Profile Interaction: a viewer (never the owner) clicking one of these
   * counts as a social-link-click. Both optional -- if creatorId isn't
   * passed, clicks simply aren't logged (no crash, just no tracking). */
  creatorId?: string; actorId?: string;
}) {
  ...
  const track = (platform: string) => {
    if (!isOwner && creatorId) logProfileEngagement(creatorId, 'social_link_click', actorId, platform);
  };
    ...
    <section className="bg-white rounded-2xl border border-gray-100 p-4">
      ...
      {!hasAny ? (
        ...
        <div className="flex flex-wrap gap-2">
          {website && (
            <a href={website.startsWith('http') ? website : `https://${website}`} target="_blank" rel="noreferrer" onClick={() => track('website')}
          ...
          {instagram && (
            <a href={`https://instagram.com/${instagram.replace('@', '')}`} target="_blank" rel="noreferrer" onClick={() => track('instagram')}
          ...
          {youtube && (
            <a href={youtube.startsWith('http') ? youtube : `https://youtube.com/${youtube}`} target="_blank" rel="noreferrer" onClick={() => track('youtube')}
          ...
          {tiktok && (
            <a href={`https://tiktok.com/${tiktok.startsWith('@') ? tiktok : '@' + tiktok}`} target="_blank" rel="noreferrer" onClick={() => track('tiktok')}
          ...
          {facebook && (
            <a href={facebook.startsWith('http') ? facebook : `https://facebook.com/${facebook}`} target="_blank" rel="noreferrer" onClick={() => track('facebook')}
          ...
          {x && (
            <a href={`https://x.com/${x.replace('@', '')}`} target="_blank" rel="noreferrer" onClick={() => track('x')}
              className="flex items-center gap-1.5 text-xs text-white bg-black hover:bg-gray-900 px-2.5 py-1 rounded-full font-medium transition-colors">
